...
import subprocess, shlex, time, re, threading, datetime, shutil
...
def latest_file(dir_path: Path, pattern: str):
    """返回目录下匹配模式的最近修改文件 Path，若无返回 None。"""
...
def validate_video(path: str) -> bool:
    """
    用 ffprobe 校验视频是否可读可用：
    - 能读取 duration 且 > 0 视为可用；
    - 任何异常或时长 <= 0 视为不可用。
    """
    try:
...
def convert_to_720(src: Path, dst: Path) -> str:
    """将 src 转为 720p 保存到 dst，返回采用的方案名。
    依次尝试：
      1) 全GPU：cuda解码 + scale_cuda + h264_nvenc（快）
      2) 混合：CPU缩放 + h264_nvenc（兼容好）
      3) 全CPU：CPU缩放 + libx264（最稳）
    统一加：-fflags +genpts、-movflags +faststart、-pix_fmt yuv420p
    """
    src_q = shlex.quote(str(src))
    dst_q = shlex.quote(str(dst))
    vf_gpu = f"scale_cuda={SIZE[0]}:{SIZE[1]},fps={FPS}"
    vf_sw  = f"scale={SIZE[0]}:{SIZE[1]}:flags=lanczos,fps={FPS}"

    # 先输出到临时文件，校验通过后原子替换，避免留下半成品
    tmp = dst.with_name(dst.name + ".tmp")
    tmp_q = shlex.quote(str(tmp))

    plans = [
        (
            "gpu",
            f"ffmpeg -hide_banner -loglevel warning -y -fflags +genpts "
            f"-hwaccel cuda -hwaccel_output_format cuda -extra_hw_frames 8 "
            f"-i {src_q} -vf {vf_gpu} -c:v h264_nvenc -preset p5 -b:v 5M "
            f"-pix_fmt yuv420p -movflags +faststart -an -f mp4 {tmp_q}"
        ),
        (
            "nvenc_swscale",
            f"ffmpeg -hide_banner -loglevel warning -y -fflags +genpts "
            f"-i {src_q} -vf {vf_sw} -c:v h264_nvenc -preset p5 -b:v 5M "
            f"-pix_fmt yuv420p -movflags +faststart -an -f mp4 {tmp_q}"
        ),
        (
            "cpu",
            f"ffmpeg -hide_banner -loglevel warning -y -fflags +genpts "
            f"-i {src_q} -vf {vf_sw} -c:v libx264 -preset medium -crf 23 "
            f"-pix_fmt yuv420p -movflags +faststart -an -f mp4 {tmp_q}"
        ),
    ]
    ...
    for name, cmd in plans:
        try:
            ...
            rm_force(tmp)
            run(cmd)
            if validate_video(str(tmp)):
                # 原子替换目标文件，避免半成品干扰下次运行
                rm_force(dst)
                Path(tmp).rename(dst)
                print(f"  ✅ 720p 转码成功（方案：{name}） → {dst.name}")
                return name
            else:
                print(f"  ⚠️ 720p 产物校验失败（方案：{name}），尝试下一方案…")
        except subprocess.CalledProcessError as exc:
            last_err = exc
            print(f"  ❌ 720p 转码失败（方案：{name}）：{exc}")
    raise RuntimeError(f"所有 720p 转码方案均失败：{src}")
...
def batch_convert_to_720p():
    """首次运行时，批量将 videos 目录下非 720p 视频转换为 720p（保留原文件）"""
    script_dir = Path(__file__).parent
    vid_dir = script_dir / "videos"
    ...
    def is_720p_file(p: Path) -> bool:
        """检测文件是否已经是720p格式，避免重复转换"""
        import re
        # 检测模式：
        # 1. 包含 .720. 或 .720.mp4 结尾的文件（包括重复转换的）
        # 2. 文件名包含 (720p) 的文件
        pattern = r'\.720(\.720)*\.mp4$|\(720p\)'
        return bool(re.search(pattern, p.name, re.IGNORECASE))
    ...
    for i, vid in enumerate(non_720p_videos, 1):
        ...
        if output_720p.exists():
            # 若已有 .720.mp4 但不可读（如 moov 缺失），尝试从原文件重建
            if not validate_video(str(output_720p)):
                print(f"  [{i}/{len(non_720p_videos)}] 发现损坏的 720p 文件，尝试重建 → {output_720p.name}")
                try:
                    run(
                        f"ffmpeg -y -hwaccel cuda -hwaccel_output_format cuda "
                        f"-i {shlex.quote(str(vid))} "
                        f"-vf scale_cuda={SIZE[0]}:{SIZE[1]} "
                        f"-c:v h264_nvenc -preset p5 -b:v 5M -an {shlex.quote(str(output_720p))}"
                    )
                    # 仅在重建成功且校验通过时删除原文件
                    if validate_video(str(output_720p)):
                        rm_force(vid)
                        print(f"  ✅ {vid.name} 重建完成，原文件已删除")
                    else:
                        print(f"  ❌ 重建后仍不可读: {output_720p.name}")
                except Exception as e:
                    print(f"  ❌ 重建失败: {e}")
                continue
            # 正常可读则跳过
        ...
        try:
            plan = convert_to_720(vid, output_720p)
            # 转换成功，删除原文件节省空间
            rm_force(vid)
            print(f"  ✅ {vid.name} 转换完成（{plan}），原文件已删除")
        except Exception as e:
...
def main():
    ...
    # 最小改动方案：
    # - 若选中的背景文件已经是 .720.mp4，则直接使用，避免反复派生 .720.720.mp4 链式文件名
    # - 否则仅转码一次为 <原名>.720.mp4，转码成功后删除原文件，与现有清理策略保持一致
    if bg_src.name.endswith('.720.mp4'):
        bg = bg_src
    else:
        bg720 = bg_src.with_name(bg_src.stem + '.720.mp4')
        if not bg720.exists():
            convert_to_720(bg_src, bg720)
        # 转码完成，删除原高分辨率视频节省空间
        rm_force(bg_src)
        print(f"🗑️ 已删除原始视频: {bg_src.name}")
        bg = bg720

    # 背景视频可读性校验；若不可用，尝试改用目录内其它可用 720p 备选
    if not validate_video(str(bg)):
        print(f"⚠️ 背景视频不可读或损坏: {bg.name}，尝试更换…")
        vid_dir = (Path(__file__).parent / 'videos')
        fallback = None
        for cand in sorted(vid_dir.glob('*.mp4')):
            if cand == bg:
                continue
            if validate_video(str(cand)):
                fallback = cand
                break
        if fallback:
            print(f"🔁 已切换到可用背景: {fallback.name}")
            bg = fallback
        else:
            raise FileNotFoundError("未找到可用的背景视频（所有 720p 文件均不可读）")

    # 音频与字幕 (自动检测)
    script_dir = Path(__file__).parent
    # 选择最新的音频作为“本次素材”
    wav_latest = latest_file(script_dir, "*.wav")
    if not wav_latest:
        raise FileNotFoundError("脚本目录下未找到 .wav 音频文件")
    audio_path = wav_latest
    ...
    # 运行前：输出目录不做清理，保留所有历史产物（按你的要求）

    # 开跑前不再删除其它输入素材（wav/srt），仅在收尾阶段统一删除本次使用的 wav/srt
    ...
    # 输入（使用GPU硬件解码加速背景视频，滤镜较多故不用cuda格式输出）
    cmd = ["ffmpeg -y", "-hwaccel cuda", "-stream_loop -1", f"-i {shlex.quote(str(bg))}", f"-i {shlex.quote(str(audio_path))}"]
    for img, *_ in images:
    ...
    # 输出（优化GPU编码参数：高质量VBR + 自适应量化）
    cmd += [
        "-filter_complex", '"'+''.join(filters)+'"',
        "-map", "[vout]", "-map", "1:a"
    ]

    # NVENC 编码策略：先尝试兼容性最佳的 VBR 单通道；若仍失败再尝试双通道高质量配置
    nvenc_plans = [
        (
            "vbr",
            "-c:v h264_nvenc -preset p4 -tune hq -rc vbr -cq 23 "
            "-b:v 2.5M -maxrate 3.5M -bufsize 5M -spatial-aq 1 -temporal-aq 1 -rc-lookahead 32 "
            "-pix_fmt yuv420p"
        ),
        (
            "vbr_hq",
            "-c:v h264_nvenc -preset slow -tune hq -multipass fullres -rc vbr_hq -cq 23 "
            "-b:v 2.5M -maxrate 3.5M -bufsize 5M -spatial-aq 1 -temporal-aq 1 -rc-lookahead 32 "
            "-pix_fmt yuv420p"
        ),
    ]
    ...
    last_err = None
    for plan_name, plan_opts in nvenc_plans:
        try:
            run(' '.join(cmd + [plan_opts] + audio_tail))
            last_err = None
            break
        except subprocess.CalledProcessError as exc:
            last_err = exc
            print(f"⚠️ NVENC 配置失败：{plan_name}，尝试备用方案…")
    if last_err:
        raise last_err
